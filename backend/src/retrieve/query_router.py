...
_EXPLAIN_PATTERN = re.compile(r"\b(?:explain|meaning|tradeoff|result|results|interpret|interpretation)\b|说明|解释|含义|结果|权衡", re.IGNORECASE)
_ERROR_PATTERN = re.compile(r"\b(?:error|exception|traceback|stack trace|failed|failure|timeout|404|500|importerror|keyerror|valueerror)\b", re.IGNORECASE)
_BROAD_PATTERN = re.compile(r"总结|比较|区别|原理|设计|概览|overview|compare|difference|design|architecture|workflow|summary", re.IGNORECASE)
_FOLLOWUP_PATTERN = re.compile(r"\b(?:it|this|that|these|those|they|them|he|she|there)\b|这个|那个|这些|那些|上面|前面|刚才", re.IGNORECASE)
_TARGET_STOPWORDS = {
    "a", "an", "the", "is", "are", "was", "were", "be", "to", "of", "for", "and", "or", "in", "on", "at", "by",
    "how", "what", "why", "when", "where", "which", "does", "do", "did", "can", "could", "should", "would",
    "please", "explain", "meaning", "tradeoff", "result", "results", "interpret", "interpretation",
    "请", "帮", "一下", "怎么", "如何", "什么", "哪个", "哪些", "以及", "或者", "还有", "和", "的", "与", "及",
    "说明", "解释", "含义", "结果", "权衡", "图", "表", "公式", "页", "节", "第",
}
...
class QueryRouter:
    ...
    def analyze(self, query: str) -> QueryAnalysis:
        ...
        has_code_signal = bool(self._contains_hard_code_signal(normalized_query)) or len(camel_tokens) >= 2
        return QueryAnalysis(
            raw_query=query,
            normalized_query=normalized_query,
            tokens=tokens,
            token_count=token_count,
            has_structure_signal=bool(self._settings.query_route_structure_pattern_enabled and _STRUCTURE_PATTERN.search(normalized_query)),
            ...
            is_followup_style=bool(_FOLLOWUP_PATTERN.search(normalized_query)) and token_count <= max(6, self._settings.query_route_exploration_token_threshold + 1),
    ...
    def route(self, query: str, *, requested_top_k: int) -> QueryRouteDecision:
        ...
        if analysis.has_structure_signal and analysis.has_explain_signal:
            return self._build_explained_structure_decision(analysis=analysis, requested_top_k=requested_top_k)
        if analysis.has_structure_signal:
            return self._build_structure_decision(analysis=analysis, requested_top_k=requested_top_k)
        if self._is_precision_query(analysis):
            return self._build_precision_decision(analysis=analysis, requested_top_k=requested_top_k)
        return self._build_general_decision(analysis=analysis, requested_top_k=requested_top_k)

    def _is_precision_query(self, analysis: QueryAnalysis) -> bool:
        return analysis.has_error_signal or analysis.has_code_signal or analysis.literal_ratio >= self._settings.query_route_precision_literal_ratio_threshold
    ...
    def _build_precision_decision(self, *, analysis: QueryAnalysis, requested_top_k: int) -> QueryRouteDecision:
        rerank_top_k = max(requested_top_k * self._settings.rerank_candidate_multiplier, requested_top_k)
        return QueryRouteDecision(
            route_name="precision",
            enable_multi_query=False,
            preserve_literals=True,
            prefer_lexical=True,
            dense_k=max(requested_top_k * 2, min(self._settings.hybrid_dense_k, max(requested_top_k * 2, 8))),
            lexical_k=max(requested_top_k * 3, min(self._settings.hybrid_lexical_k, max(requested_top_k * 3, 10))),
    ...
    def _build_structure_decision(self, *, analysis: QueryAnalysis, requested_top_k: int) -> QueryRouteDecision:
        rerank_top_k = max(requested_top_k * self._settings.rerank_candidate_multiplier, requested_top_k)
        return QueryRouteDecision(
            ...
            prefer_lexical=True,
            dense_k=max(requested_top_k * 2, min(self._settings.hybrid_dense_k, max(requested_top_k * 2, 8))),
            lexical_k=max(requested_top_k * 4, min(self._settings.hybrid_lexical_k, max(requested_top_k * 4, 12))),
            fusion_top_k=max(requested_top_k * 2, requested_top_k),
            rerank_top_k=rerank_top_k,
    ...
    def _build_explained_structure_decision(self, *, analysis: QueryAnalysis, requested_top_k: int) -> QueryRouteDecision:
        rerank_top_k = max(requested_top_k * self._settings.rerank_candidate_multiplier, requested_top_k)
        return QueryRouteDecision(
    ...
    def _build_general_decision(self, *, analysis: QueryAnalysis, requested_top_k: int) -> QueryRouteDecision:
        rerank_top_k = max(requested_top_k * self._settings.rerank_candidate_multiplier, requested_top_k)
        return QueryRouteDecision(
            route_name="general",
            enable_multi_query=False,
            ...
            target_section_terms=self._extract_target_section_terms(analysis),
            enforce_targeting=False,
            retry_policy="widen_only" if self._settings.targeted_retry_enabled else "none",
        )
    ...
    def _contains_hard_code_signal(text: str) -> bool:
        return bool(re.search(r"[/\\]|::|->|[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z0-9_.]+|[A-Za-z_][A-Za-z0-9_]*\(|[A-Za-z_][A-Za-z0-9_]*_[A-Za-z0-9_]+", text))

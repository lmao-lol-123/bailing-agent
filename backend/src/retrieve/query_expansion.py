...
_STOPWORDS = {
    ...
    "请", "帮", "一下", "怎么", "如何", "什么", "哪个", "哪些", "以及", "或者", "还有", "和", "的", "与", "及", "比较",
    "说明", "解释", "含义", "结果", "权衡",
}
...
class DeterministicQueryExpander:
    ...
    def expand(self, *, query: str, analysis: QueryAnalysis, decision: QueryRouteDecision) -> list[QueryVariant]:
        ...
        if not self._settings.query_multi_enabled or not decision.enable_multi_query or decision.max_query_variants <= 1:
            return variants
        ...
        if keyword_text and keyword_text.lower() != original_query.lower():
            variants.append(QueryVariant(variant_id="keyword_focused", text=keyword_text, kind="keyword_focused", weight=0.92))

        if decision.route_name == "explained_structure":
            clarified_text = self._build_clarified(original_query=original_query, keyword_text=keyword_text)
            if clarified_text and clarified_text.lower() not in {variant.text.lower() for variant in variants}:
                variants.append(QueryVariant(variant_id="clarified", text=clarified_text, kind="clarified", weight=0.85))
    ...
    def build_retry_variant(self, *, query: str, analysis: QueryAnalysis) -> QueryVariant | None:
        ...
        if not keyword_text or keyword_text.lower() == normalized_query.lower():
            return None
        return QueryVariant(variant_id="retry_keyword", text=keyword_text, kind="retry_keyword", weight=0.84)
